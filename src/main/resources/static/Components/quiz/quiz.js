...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var showdetail = document.getElementById("Hoverfordetail");
    var detaild = document.getElementById("hovereddisplay");
    var questionCount = 2;
    // Event listener for clicks on the document
    showdetail.addEventListener('mouseover', function () {
        detaild.style.display = 'flex';
    });

    // Event listener for mouseout on the showdetail element (optional)
    showdetail.addEventListener('mouseout', function () {
        detaild.style.display = 'none'; // You might want to hide the element on mouseout
    });
    ...
    function addMore() {
        ...
        label.setAttribute('for', 'answerInput' + (choiceContainer.children.length + 1));
        label.textContent = (choiceContainer.children.length + 1) + '. ';

        var input = document.createElement('input');
        input.classList.add('choice-answer');
        input.setAttribute('type', 'text');
        input.setAttribute('id', 'answerInput' + (choiceContainer.children.length + 1));
        input.setAttribute('name', 'answerInput' + (choiceContainer.children.length + 1));

        var checkbox = document.createElement('input');
        checkbox.setAttribute('type', 'checkbox');
        checkbox.setAttribute('id', 'correctCheckbox' + (choiceContainer.children.length + 1));
        checkbox.setAttribute('name', 'correctCheckbox' + (choiceContainer.children.length + 1));
    ...
    document.getElementById('RemoveInputButton').addEventListener('click', removeInput);
    ...
    function removeInput() {
        ...
        if (choiceContainer.children.length >= 2) {
            ...
            if (choiceContainer.children.length < 4) {
                ...
                document.getElementById('addInputButton').style.display = 'inline-block';
            }
